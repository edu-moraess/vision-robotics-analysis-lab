"""Video file as a CameraSource (offline replay)."""
...
class VideoFileSource(CameraSource):
    ...
    def read(self) -> Optional[FramePacket]:
        ...
        if not ok or frame is None:
            if self.loop and self._cap is not None:
                ...
                ok, frame = self._cap.read()
            if not ok or frame is None:
                self._message = "end of video / read failure"
                return None
        now = time.time()
        if self._last_ts > 0:
        ...
        self._frame_id += 1
        self._message = "online"
        return FramePacket(image=frame, timestamp=now, frame_id=self._frame_id, source=f"video:{self.path[-32:]}")
    ...
    def status(self) -> CameraStatus:
        ...
        return CameraStatus(online=self.is_available(), source=f"video:{self.path[-40:]}", message=self._message, resolution=res, measured_fps=self._fps_ema)
